...
describe 'gpfs::client::rkm' do
  on_supported_os({
    ...
    context "on #{os}" do
      ...
      it 'should manage key store parent directory' do
        is_expected.to contain_file('/var/mmfs/etc/RKMcerts').with({
          :ensure => 'directory',
          :owner  => 'root',
          :group  => 'root',
          :mode   => '0755',
      ...
      it 'should manage key store' do
        is_expected.to contain_file('/var/mmfs/etc/RKMcerts/ISKLM.proj2').with({
          :ensure => 'file',
          :owner  => 'root',
          :group  => 'root',
          :mode   => '0600',
