...
describe Puppet::Type.type(:gpfs_fileset).provider(:rest_v2) do
  ...
  describe 'self.instances' do
    it 'should create instances' do
      allow(@provider).to receive(:get_request).with("v2/filesystems/:all:/filesets", {'fields' => ':all:'}).and_return(all_filesets_data)
      expect(@provider.instances.length).to eq(3)
    end

    it 'should create no instance when no filesets returned' do
      allow(@provider).to receive(:get_request).with("v2/filesystems/:all:/filesets", {'fields' => ':all:'}).and_return({"filesets" => []})
      expect(@provider.instances.length).to be_zero
    end

    it 'should return the resource for a fileset' do
      allow(@provider).to receive(:get_request).with("v2/filesystems/:all:/filesets", {'fields' => ':all:'}).and_return(all_filesets_data)
      expect(@provider.instances[0].instance_variable_get("@property_hash")).to eq(all_filesets[0])
    end
  end
  ...
  describe 'flush' do
    it 'should send PUT request with data' do
